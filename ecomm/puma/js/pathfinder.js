...
// finally, this stuff added to support 3D pathfinding
...
var Pathfinder = function () {
	...
	this.addNode = function (id, x, y, z) {
		if (!nodes[id]) {
	...
	this.addEdge = function (id1, id2) {
		var edgeA = id1 + '=' + id2;
		if (edges[edgeA]) {
			edges[edgeA].perimeter = false;
		} else {
			var edgeB = id2 + '=' + id1;
			if (edges[edgeB]) {
		    	edges[edgeB].perimeter = false;
			} else {
				graph.addMutualArc(nodes[id1].node, nodes[id2].node, 1);
				edges[edgeA] = {
					a: id1, b: id2,
					perimeter: true
				}
	...
	this.weld = function (thresholdSquared) {
		var cache = {};

		// we have basic volume in -1...1 range at each side,
		// split into 100^3 cubes: i = 50 * (x + 1)
		var side = 100, halfSide = Math.floor (side / 2), id = 1;

		var calculateHash = function (i, j, k) {
			return side * (side * i + j) + k;
		};

		var cacheVertex = function (id, x, y, z) {
			var i = Math.floor (halfSide * (x + 1)), j = Math.floor (halfSide * (y + 1)), k = Math.floor (halfSide * (z + 1));
			for (var i1 = i - 1; i1 <= i + 1; i1++) for (var j1 = j - 1; j1 <= j + 1; j1++) for (var k1 = k - 1; k1 <= k + 1; k1++) {
				var hash = calculateHash (i1, j1, k1);
				var bucket = cache[hash] || []; cache[hash] = bucket;
				bucket.push (id);
			}
		};
		...
		for (var e in edges) {
			...
			if (e.perimeter) {
				if(!pnodes[e.a]) {
					pnodes[e.a] = true; v = nodes[e.a]; cacheVertex (e.a, v.x, v.y, v.z);
				}
				if(!pnodes[e.b]) {
					pnodes[e.b] = true; v = nodes[e.b]; cacheVertex (e.b, v.x, v.y, v.z);
				}
			}
		}

		var stats = 0, perimeter = Object.keys (pnodes);
		for (var p = 0; p < perimeter.length; p++) {
			var id1 = perimeter[p]; v = nodes[id1];

			var i = Math.floor (halfSide * (v.x + 1)), j = Math.floor (halfSide * (v.y + 1)), k = Math.floor (halfSide * (v.z + 1));
			var hash = calculateHash (i, j, k);
			var bucket = cache[hash];

			for (i = 0; i < bucket.length; i++) {
				var id2 = bucket[i]; w = nodes[id2];

				if (id1 == id2) continue;

				if (this.hasEdge (id1, id2)) continue;

				var dx = v.x - w.x, dy = v.y - w.y, dz = v.z - w.z;
				if (dx * dx + dy * dy + dz * dz < thresholdSquared) {
					this.addEdge (id1, id2); stats++;
				}
			}
		}

		console.log('pathfinder.weld( ' + thresholdSquared + ' ): graph perimeter length was ' + perimeter.length + ', added ' + stats + ' edges along it.');
	};

	this.findPath = function (id1, id2) {
		var path = new de.polygonal.ds.DA();
		if (astar.find(graph, nodes[id1], nodes[id2], path)) {
			return path._a;
		}
	};
