function createAvatarMaterial (material, textures, hasBones, envMap) {
    ...
    if(!specular) {
        specular = THREE.ImageUtils.generateDataTexture(4, 4, new THREE.Color(0x7f807e));
        if (material.textures[2]) console.log ('Texture not found: ' + material.textures[2]);
    }

    var roughness = textures[ material.textures[3] ];
    if(!roughness) {
        roughness = THREE.ImageUtils.generateDataTexture(4, 4, new THREE.Color(0xffffff));
        if (material.textures[3]) console.log ('Texture not found: ' + material.textures[3]);
    }
    ...
    if ((material.VertexProgramName == "MVVertexShaderV3")  && (material.textures[0]=="HairDiffuse.png"))
    {result.side = THREE.DoubleSide;
        result.transparent = true;
    }

    if ((material.VertexProgramName == "MVVertexShaderV3")  && (material.textures[0]=="eyes_OP.png"))
    {result.side = THREE.DoubleSide;
        result.transparent = true;
    }

    switch (material.VertexProgramName) {
        case 'sceneBase':
            // мне нужно, что бы сцена была не матовая, цвет для рафнес 464646
            result.roughness = 0x46 / 255.0;
            result.side = THREE.DoubleSide;
            result.transparent = true;
            break;
        ...
        case 'eyelashesVP':
            ...
            if (material.textures[0] == "eyes_OP.png") {
                result.transparent = true;
