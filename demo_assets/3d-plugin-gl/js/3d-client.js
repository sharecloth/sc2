function Plugin3d(container, options) {
    var settings = $.extend( {
        ...
        'disablePitch' : false,
        // предел прозрачности волос (в настоящее время не используется)
        'hairThreshold' : 0.5,
    ...
    function init() {
        ...
        */
        ...
        THREE.ShaderLib.phong.vertexShader =
            ...
            THREE.ShaderLib.phong.vertexShader.substr(0, THREE.ShaderLib.phong.vertexShader.lastIndexOf('}')) +
            'v_gl_TexCoord_1 = uv1;\n' +
            'v_gl_TexCoord_2 = uv3;\n' +
            '}';
        ...
        holder.add (camera); scene.add (holder);
        ...
        subcontainer.append(
            $('<div class="controls">' +
                ...
                '<span class="fs"></span>' +
                (WEBVR.isAvailable() ? '<span class="vr"></span>' : '') +
            ...
            }).on('mouseup', sfs, function () { // can't use list of events for fullscreen, fu jquery
                toggleFullScreen (container[0]);

            }).on('touchend', sfs, function () {
                toggleFullScreen (container[0]);

            }).on('mouseup', svr, function () {
                vrEffect.isPresenting ? vrEffect.exitPresent() : vrEffect.requestPresent(); setInitialCameraPosition ();

            }).on('touchend', svr, function () {
                vrEffect.isPresenting ? vrEffect.exitPresent() : vrEffect.requestPresent(); setInitialCameraPosition ();
...
*/
    ...
    var processTextureFile = function (zip, fileName) {
        ...
        var url = 'data:image/' + ext + ';base64,' + JSZip.base64.encode (zip.file (fileName).asBinary ());
    ...
    this.loadScene = function (sceneId, callback) {
        ...
        $.when(
            loadZip (pathResolver.getScenePath(sceneId, settings), function (zip, fileName) {
                return processMeshFile (zip, fileName) || processTextureFile (zip, fileName);
            })
        ).then(function (files) {
    ...
    this.loadDummy = function(dummyId, callback) {
        ...
        $.when(
            $.getJSON (pathResolver.getAvatatParamsPath(dummyId, settings)),
            loadZip (pathResolver.getAvatarDataPath(avatarId, settings), processMeshFile),
            loadZip (pathResolver.getAvatarTexturePath(avatarId, settings), processTextureFile)
            ...
            for (var name in meshes) {
                ...
                for (var i = 0; i < robemesh.geometries.length; i++) {
                    ...
                    // override shitty avatar material params
                    material.Ambient = { r: 0.40, g: 0.35, b: 0.33, a: 1 };
                    material.Diffuse = { r: 0.94, g: 0.94, b: 0.94, a: 1 };

                    var diffuse = textures[ material.textures[0] ];
                    if(!diffuse) {
                        diffuse = THREE.ImageUtils.generateDataTexture(4, 4, new THREE.Color(0xafafaf));
                        if (material.textures[0]) console.log ('Texture not found: ' + material.textures[0]);
                        else console.log('No texture name for ' + name + ' geometry ' + i);
                    }

                    var normal = textures[ material.textures[1] ];
                    if(!normal) {
                        normal = THREE.ImageUtils.generateDataTexture(4, 4, new THREE.Color(0x8080ff));
                        if (material.textures[1]) console.log ('Texture not found: ' + material.textures[1]);
                    }

                    var specular = textures[ material.textures[2] ];
                    if(!specular) {
                        specular = THREE.ImageUtils.generateDataTexture(4, 4, new THREE.Color(0));
                        if (material.textures[2]) console.log ('Texture not found: ' + material.textures[2]);
                    }

                    var hasBones = (geometry.bones.length > 0);
                    var mesh = new THREE.SkinnedMesh (geometry,
//                        new MVMaterial (material, null, textures, hasBones)
                    ...
                    if (/\.png$/.test (material.textures[0])) {
                        if (/hair/i.test (material.textures[0])) {
                            mesh.material.side = THREE.DoubleSide;
//                            mesh.material.uniforms.threshold.value = settings.hairThreshold;
                            //mesh.material.alphaTest = settings.hairThreshold;
                    ...
                    if (hasBones) {
                        if (debug) {
                            ...
                            scene.add (window.sh = new THREE.SkeletonHelper (mesh));
                        }
    ...
    this.loadProducts = function (combinationIds, callback, clothTypeId) {
        ...
        if (!combinationIds || (combinationIds == '')) {
        ...
        var args = [
            loadZip(pathResolver.getMesh(combinationIds, avatarId, settings), processMeshFile)
        ];

        //todo uncompleted pathResolver - only one cloth is supported now!
        args.push (
            loadZip (pathResolver.getMeshTextures(combinationIds, settings), processTextureFile)
        );
        // combinationIds.toString ().split ('_').forEach (function (combinationId) {
        //     args.push (
        //         loadZip (pathResolver.getMeshTextures(combinationId, settings), processTextureFile)
        //     );
        // });
    ...
    var transformedSkinVertex = (function () {
        ...
        return function (skin, index) {
            var skinIndices = siv.fromAttribute (skin.geometry.getAttribute ('skinIndex'), index);
            var skinWeights = swv.fromAttribute (skin.geometry.getAttribute ('skinWeight'), index);
            var skinVertex = svv.fromAttribute (skin.geometry.getAttribute ('position'), index).applyMatrix4 (skin.bindMatrix);
            result.set (0, 0, 0);
            for (var i = 0; i < 4; i++) {
                    ...
                    scene.traverse (function (object) {
                        if (object.geometry && (object.name.indexOf ('cloth') == 0)) {
                            ...
                            for (var i = 0, n = indicesArray.length; i < n; i += 3) {
                                triangle.a.fromAttribute (positions, indicesArray[i]); object.localToWorld (triangle.a);
                                triangle.b.fromAttribute (positions, indicesArray[i + 1]); object.localToWorld (triangle.b);
                                triangle.c.fromAttribute (positions, indicesArray[i + 2]); object.localToWorld (triangle.c);
